...
public class Salle {
    ...
    public void finirSeance() throws ErreurSeanceEnCours {
		if (indiceSeanceEnCours == -1)throw new ErreurSeanceEnCours("BOOOOO");
		if (this.indiceSeanceEnCours == this.seances.size() - 1) {
			setSeanceEnCours(-1);
		}
		else {
			setSeanceEnCours(indiceSeanceEnCours+1);
		}
	}
    ...
    public void acheterSeanceEnCours(int nbBillets) throws NombrePlacesErreur, ErreurSeanceEnCours {
		if(indiceSeanceEnCours == -1 ) throw new ErreurSeanceEnCours("BOOO");
		Seance sEnCours = seances.get(indiceSeanceEnCours);
    	if(sEnCours.getNbPlacesDispo() < nbBillets) throw new NombrePlacesErreur("Ya pas wesh");
    	sEnCours.setPlacesDisponible(sEnCours.getNbPlacesDispo()-nbBillets);
